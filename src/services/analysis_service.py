...
class AnalysisService(object):
    ...
    def analyse_real_time(self, batch: list[np.ndarray], processing_config: "ProcessingConfig") -> AnalysisResult:
        # Order per channel
        data = {}
        for p in batch:
    ...
    def _create_batch(self, events: List[dict[str, Any]], processing_config: ProcessingConfig, channel_id: int) -> BatchPulses:
        pulses_list = []
        for event in events:
    ...
    def legacy_analyse_csv_file(self, path: str, analysis_config: "AnalysisConfig | None" = None) -> AnalysisResult | None:
        """Analyse the data contained in a CSV file using the default ProcessingConfig.\n
        WARNING: This works only for the Bergoz BCT with CeanDT5781 since it was exclusively used by FLASHy 1.0.X"""
